    ...
    const validationOptions=[
        {
          ...
          errorMessage:(input,label)=>`${label.textContent} need to be least ${input.minLength} characters`,
        },
        {
            attribute:'custommaxlength',
            isValid:input=>input.value&&input.value.length<=parseInt(input.getAttribute('custommaxlength'),10),
            errorMessage:(input,label)=>`${label.textContent} must be less than ${input.getAttribute('custommaxlength')} character`,
        },
        {
            attribute:'pattern',
            isValid:input=>{
